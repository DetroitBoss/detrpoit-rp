...
class SystemClass extends systemUtil {
    static getHeadingBetweenCoords(from: Vector2, to: Vector2){
        let x = to.x - from.x
        let y = to.y - from.y
    ...
    static headingToCoord(pos1: {x: number, y: number}, pos2: {x: number, y: number}){
    ...
    static invoke(name: keyof typeof SystemClass.natives, ...args: any[]){
        return mp.game.invoke(this.natives[name], ...args);
    }
    static invokeVector3(name: keyof typeof SystemClass.natives, ...args: any[]){
    ...
    static interriorBlips: { type: number, color: number, position: Vector3Mp, name: string, dimension:number, shortRange: boolean}[] = []
    static interriorBlipsEnt: BlipMp[] = []
    static createBlipInterrior(type: number, color: number, position: Vector3Mp, name: string, dimension = 0, shortRange: boolean = false) {    
        SystemClass.interriorBlips.push({type, color, name, dimension, position, shortRange})
    }
    static nearestBlips: { type: number, color: number, position: Vector3Mp, name: string, dimension:number, shortRange: boolean, range: number, created: boolean}[] = []
    static nearestBlipsEnt: [BlipMp, number][] = []
    static createBlipNearest(type: number, color: number, position: Vector3Mp, name: string, range: number, dimension = 0, shortRange: boolean = false) {    
        SystemClass.nearestBlips.push({ type, color, name, dimension, position, shortRange, range, created: false})
    }
    static createDynamicBlip(id: string, type: number, color: number, position: {x: number, y: number, z: number}, name: string, options?: DynamicBlipOption) {
        return new DynamicBlip(id, type, color, position, name, options)
    }
    static createPed(pos: Vector3Mp, heading: number, model: HashOrString, dimension = 0) {
...
function recreate(){
    SystemClass.interriorBlipsEnt.map((blip, index) => {
        if (mp.blips.exists(blip)){
            if (!user.inInterrior || system.distanceToPos2D(player.position, blip.getCoords()) > 300) blip.destroy(), SystemClass.interriorBlipsEnt.splice(index, 1)
        }  
    })
    if (user.inInterrior && !inInt){
        inInt = true;
    ...
    } else if (!user.inInterrior) inInt = false;
...
CustomEvent.register('enterInterrior', () => {
    recreate()
})
CustomEvent.register('exitInterrior', () => {
    recreate()
})

SyncClass.handle('serverReboot', (val: number) => {
    rebTm(val);
})

const rebTm = (time: number) => {
    if (timerReboot && timerReboot.exists) timerReboot.destroy();
    if(time) timerReboot = hudBar.timer('Перезагрузка сервера', time);
    else timerReboot = null;
}

let timerReboot: TextTimerBar;
CustomEvent.registerServer('server:reboot:timer', (time: number) => rebTm(time))
