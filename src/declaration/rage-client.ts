...
interface GameMp {
  gxt: {
    ...
    reset: (labelNameOrHash?: HashOrString)=>void;
  };
  app: GameAppMp;
  audio: GameAudioMp;
  brain: GameBrainMp;
  ...
  invokeVector3(hash: string, ...args: any[]): {x: number, y: number, z: number};
...
interface PedMp extends EntityMp {
  ...
  getLastDamageBone(damageAmount: number, p2: boolean): void;
  taskPlayAnim(
...
interface VehicleMp extends EntityMp {
  ...
  getDefaultHandling(handlingName: string): number;
  ...
  getHandling(handlingName: string): number;
  ...
  setHandling(typeName: string, value: number | string, applyGlobal?: boolean): void;
...
interface StorageMp {
  data: {
    renderFamilyPlayerBlips: boolean;
    lodDistPlayers: number;
    lodDistVehs: number;
    boomboxSound: number;
    alertsEnable: Partial<import('../shared/alertsSettings').StorageAlertData>
    radioFreq?: string,
    lastVehicle: string;
    voiceUsers: [number, number][];
    voiceSettings: [number, number, number];
    hotkeys: typeof import("../shared/hotkeys").defaultHotkeys;
    voiceLevel: number,
    crosshair: import('../shared/crosshair').ICrosshairSettings
    [key: string]: any 
  };
...
interface GamePlayerMp {
  ...
  getEntityIsFreeAimingAt(): number | EntityMp;
...
interface GameUiMp {
  ...
  notifications: {
    ...
    showWithPicture(title: string, sender: string, message: string, icon?: string, time?:number): any
  }
}
...
interface GameWeaponMp {
  ...
  getWeaponComponentTypeModel(componentHash: Hash): number;
...
interface EventMpPool {  
  ...
  add(eventName: 'playerEnterColshape', callback: (player: PlayerMp, colshape: ColshapeMp)=>void): void;
  add(eventName: 'playerEnterColshape', callback: (colshape: ColshapeMp)=>void): void;
